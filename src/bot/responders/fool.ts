...
export namespace Fool {
  export function initialize(): void {
    Allocater.entryResponder(
      `${COMMAND_PREFIX}fakepoll`, chunk => respond(chunk)
    );
  }

  async function respond(chunk: RequestChunk): Promise<Message | null> {
    if (!validate(chunk)) return null;
  ...
  function validate(chunk: RequestChunk): boolean {
  ...
  async function parseChoices(
    poll: Message, additions: number[]
  ): Promise<Choice[]> {
  ...
  function parseAdditions(chunk: RequestChunk): number[] {
    return chunk.args.slice(1).map(addition => {
      addition = addition.replace(
        /[０-９]/g, num => String.fromCharCode(num.charCodeAt(0) - 0xFEE0)
      ).replace(/[ー―－‐]/, '-');
